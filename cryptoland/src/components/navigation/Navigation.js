...
import { NavigationContainer, HamburgerMenuContainer } from "../../styles";
...
class Navigation extends Component {
  ...
  handleNavbarLocation = () => {
    ...
    const isInHero =
      window.scrollY < heroHeight - (navbarHeight + tickerHeight);
  ...
  render() {
    ...
    const menuItems = menu.map((item, i) => {
      ...
      if (this.props.width <= 1200) {
        // --- Mobile menu ---
        return (
          <MenuItem key={i} delay={`${i * 0.1}s`}>
            {item === "LOGIN" ? (
              <button onClick={() => window.goToDashboard()}>{item}</button>
            ) : (
              <Link to={`/${itemLink}`} onClick={this.handleLinkClick}>
                <p>{item}</p>
              </Link>
            )}
          </MenuItem>
        ...
        if (item !== "LOGIN") {
          return (
            <Button
              ...
              noBackground
              textColor={`${
                this.props.location.pathname === `/${itemLink}`
                  ? "#55E2BE"
          ...
          return (
            <Button
              ...
              pPadding="10% 5%"
              textColor={
                this.props.location.pathname === "/" ? "white" : "#808080"
              }
              fontSize=".9em"
              fontWeight="900"
              letterSpacing="1.5px"
        ...
        {this.props.width <= 1200 ? (
          ...
          >
            <img
              ...
              src={
                this.props.location.pathname === "/"
                  ? "./assets/images/Cryptoland_Logo_Green_Icon.png"
                  : "./assets/images/Cryptoland_Logo_ALL_Green.png"
              }
              alt="Cryptoland logo"
            />
            <div>{menuItems}</div>
